...
expect.extend({
    toBeSecurePassword(received) {
        ...
        if (!isLongEnough)
            failureReasons.push(`minimum ${minLength} characters`);
        if (!hasUpperCase)
            failureReasons.push('uppercase letter');
        if (!hasLowerCase)
            failureReasons.push('lowercase letter');
        if (!hasNumbers)
            failureReasons.push('number');
        if (!hasSpecialChar)
            failureReasons.push('special character');
        if (pass) {
            return {
...
expect.extend({
    toNotExposeInternalInfo(received) {
...
expect.extend({
    toHaveRateLimitHeaders(received) {
        ...
        const presentHeaders = rateLimitHeaders.filter(header => received.headers[header] !== undefined);
...
expect.extend({
    toBeSecureFileUpload(received) {
        ...
        if (!received.filename) {
            ...
            const hasUnsafeExtension = dangerousExtensions.some(ext => received.filename.toLowerCase().endsWith(ext));
...
expect.extend({
    toPreventInjection(received) {
        ...
        const containsSqlKeywords = sqlKeywords.some(keyword => body.toUpperCase().includes(keyword));
        ...
        const containsCommandIndicators = commandIndicators.some(indicator => body.includes(indicator));
...
expect.extend({
    toHaveCSPHeader(received) {
        ...
        const missingDirectives = requiredDirectives.filter(directive => !cspHeader.includes(directive));
...
expect.extend({
    toPreventXSS(received) {
        ...
        const hasEventHandlers = eventHandlers.some(handler => body.toLowerCase().includes(handler));
        ...
        if (pass) {
            ...
            const issues = [];
            if (hasScriptTags)
                issues.push('script tags');
            if (hasEventHandlers)
                issues.push('event handlers');
            if (hasJavascriptUrls)
                issues.push('javascript URLs');
            if (hasDataUrls)
                issues.push('data URLs with script');
            return {
...
expect.extend({
    toHaveSecureSessionConfig(received) {
        const issues = [];
...
//# sourceMappingURL=security-matchers.js.map
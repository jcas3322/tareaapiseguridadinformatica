...
const BcryptPasswordHasher_1 = require("../../../../src/infrastructure/security/BcryptPasswordHasher");
describe('BcryptPasswordHasher Integration Tests', () => {
    let passwordHasher;
    beforeEach(() => {
        passwordHasher = new BcryptPasswordHasher_1.BcryptPasswordHasher(10); // Use lower rounds for faster tests
    });
    describe('hash', () => {
        ...
        it('should throw error for non-string password', async () => {
            await expect(passwordHasher.hash(null)).rejects.toThrow('Password must be a non-empty string');
            await expect(passwordHasher.hash(123)).rejects.toThrow('Password must be a non-empty string');
        });
    ...
    describe('constructor validation', () => {
        it('should throw error for salt rounds below minimum', () => {
            expect(() => new BcryptPasswordHasher_1.BcryptPasswordHasher(9)).toThrow('Salt rounds must be at least 10 for security');
        });
        it('should throw error for salt rounds above maximum', () => {
            expect(() => new BcryptPasswordHasher_1.BcryptPasswordHasher(16)).toThrow('Salt rounds cannot exceed 15 for performance');
        });

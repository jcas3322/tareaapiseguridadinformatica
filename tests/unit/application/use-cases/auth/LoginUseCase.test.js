...
const LoginUseCase_1 = require("../../../../../src/application/use-cases/auth/LoginUseCase");
...
describe('LoginUseCase', () => {
    ...
    beforeEach(() => {
        ...
        useCase = new LoginUseCase_1.LoginUseCase(mockUserRepository, mockSecurityDomainService, mockPasswordHasher, mockJwtService, mockEventPublisher, mockLogger);
    });
    describe('execute', () => {
        const validCommand = {
        ...
        it('should successfully login with valid credentials', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('User logged in successfully', expect.objectContaining({
        ...
        it('should log suspicious login activity', async () => {
            ...
            // Assert
            expect(mockLogger.warn).toHaveBeenCalledWith('Suspicious login detected', expect.objectContaining({
                userId: mockUser.id.value,
                reasons: ['Multiple IP addresses', 'High failure rate']
            }));
            expect(mockLogger.security).toHaveBeenCalledWith('Suspicious login attempt', expect.any(Object));
        });
        ...
        it('should find user by username when email format is invalid', async () => {
            ...
            // Assert
            expect(result).toBeDefined();
            expect(mockUserRepository.findByUsername).toHaveBeenCalledWith(expect.objectContaining({ value: 'testuser' }));
        });
    });
});
